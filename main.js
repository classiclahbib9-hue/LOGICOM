...
ipcMain.handle('bulk-send-sold-message', async (event, { filter, template, channel }) => {
  ...
  for (const c of clients) {
    ...
    if (balance <= 0) {
      console.log(`[BulkSold] Skipping ${c.name} because balance is 0.00`);
      continue;
    }
...
ipcMain.handle('send-whatsapp-message', async (event, { phone, message, clientId }) => {
  const { sendWhatsApp, isWhatsAppReady } = require('./whatsapp');
  // Permissive check
  ...
  if (clientId) {
    ...
    const today = new Date().toLocaleDateString('fr-FR');
    if (db) {
      db.run(`UPDATE clients SET dateDernierRappel=? WHERE id=?`, [today, clientId]);
      const { saveToFile } = require('./db');
      saveToFile();
    }
  }
...
ipcMain.handle('check-whatsapp-number', async (event, { phone }) => {
  const { checkWhatsAppNumber } = require('./whatsapp');
  try {
    const isRegistered = await checkWhatsAppNumber(phone);
    return { isRegistered };
  } catch (err) {
...
ipcMain.handle('reset-whatsapp', async () => {
  const { resetWhatsApp } = require('./whatsapp');
  
  const onQrCallback = (qr) => {
    const wins = BrowserWindow.getAllWindows();
    if (wins.length) wins[0].webContents.send('whatsapp-qr', qr);
  };
  
  const onAdminNotifyCallback = async ({ channel, clientName, clientPhone, parsed, rawMessage }) => {
    const { getBot } = require('./telegram');
    const bot = getBot();
    const cfg = (() => {
  ...
  await resetWhatsApp(onQrCallback, onAdminNotifyCallback);
  return { ok: true };
});

ipcMain.handle('start-bulk-whatsapp', async (event, data) => {
  console.log('[Main] start-bulk-whatsapp received for', data.clients ? data.clients.length : 0, 'clients');
  const { processBulkWhatsApp } = require('./whatsapp');
  processBulkWhatsApp(data.clients, data.template, event);
  return { started: true };
});

ipcMain.handle('stop-bulk-whatsapp', async () => {
  const { stopBulkWA } = require('./whatsapp');
  stopBulkWA();
  return { stopped: true };
});
...
ipcMain.handle('bulk-send-reminder-message', async (event, { clientIds, template, channel }) => {
  ...
  const failedClientsList = []; // Array of { name, phone, error }
  ...
  for (const c of rows) {
    ...
    if (balance <= 0) {
      console.log(`[BulkSend] Skipping ${c.name} because balance is 0.00`);
      continue;
    }
    const msg = template
      .replace(/\{name\}/g, c.name || '')
    ...
    if ((channel === 'wa' || channel === 'both') && c.phone) {
      if (isWhatsAppReady()) {
        try { 
          ...
          failedWA++; 
          failedClientsList.push({ name: c.name, phone: c.phone, error: e.message });
        }
      } else {
        console.warn(`[BulkSend] WA Not Ready - Skipping ${c.name}`);
        failedWA++;
        failedClientsList.push({ name: c.name, phone: c.phone || 'Pas de numéro', error: 'WhatsApp non prêt / déconnecté' });
      }
    } else if ((channel === 'wa' || channel === 'both') && !c.phone) {
      failedWA++;
      failedClientsList.push({ name: c.name, phone: 'Pas de numéro', error: 'Numéro de téléphone manquant' });
    }
  ...
  return { total: rows.length, sentWA, sentTG, failedWA, failedTG, failedClientsList };
});
